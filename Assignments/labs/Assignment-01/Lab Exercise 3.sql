...
-- Task 4
-- 
-- A developer has written a query to retrieve two columns (city and country) from the Sales.Customers table. When the query is executed, it returns only one column. Your task is to analyze the query, correct it to return two columns, and explain why the query returned only one column.
--
